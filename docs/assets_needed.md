...
| 캐릭터 | 경로 | 크기 | 상태 | 필요 애니메이션 |
|--------|------|------|------|----------------|
| 사티로스 (일반) | `characters/enemies/satyr/` | 80×80px | 플레이스홀더 | idle, attack, hurt, death |
| 사티로스 (엘리트) | `characters/enemies/satyr/` | 동일 씬 재사용 | — | — |
| 보스 사티로스 | `characters/enemies/satyr/` | 동일 씬 재사용 | — | 크기만 1.5× |

> **참고:** 현재 사티로스 1종 씬을 일반/엘리트/보스 모두 재사용. 향후 보스는 별도 씬 분리 예정.
...
| 요소 | 플레이스홀더 방식 |
|------|----------------|
| 캐릭터 | `ColorRect` (나폴레옹=파랑, 사티로스=빨강) + `AnimationPlayer`(빈 트랙) |
