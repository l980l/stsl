...
> 작성일: 2026-04-17 (최종 동기화: 2026-04-21)
> 기준: 현재 구현 상태(Act 1~3 플레이 가능 + 신화 랜덤화) 기준 완성까지의 전 작업 목록
> 범례: ✅ 완료 / 🔲 미완료 / 🔶 부분 완료
...
Act 1~3 플레이 가능 + 신화 랜덤화 수직 슬라이스 완성.
영웅 3인, 영웅당 카드 40장(아키타입 3개).
**Act 1 (그리스)**: 적 10종(일반 6·엘리트 4·보스 히드라), 이벤트 10종, 렐릭 19종.
**Act 2 (이집트)**: 적 10종(일반 6·엘리트 3·보스 오시리스 2페이즈), 이벤트 10종, 전용 렐릭 3종, 난이도 스케일(HP ×1.3, 공격 ×1.2).
**Act 3 (북유럽)**: 적 10종(일반 6·엘리트 3·보스 요르문간드르 3페이즈), 이벤트 10종, 전용 렐릭 3종(총 풀 25종).
**신화 랜덤화**: 매 런마다 Act 1·2·3 신화 배정 셔플 (3! = 6가지). 전 신화 × 전 Act 엘리트 3 + 보스 1 풀 채움(9개 모듈).
크로스 시너지 3쌍, Morale/Formation/Charm 시스템 완료. HP 1000 스케일(PR #30).
기획자 도감 자동 생성기(PR #32), 전역 디버그 단축키 7종(PR #34~#35).
...
- ✅ 전역 디버그 단축키 7종 (PR #34~#35)
  - 전투 전용: Shift+I(무적) / Shift+E(무한 코스트) / Shift+D(드로우) / Shift+H(적 HP 설정)
...
### 4-1. 메타 진행 저장 시스템
> 현재 `save_manager.gd`는 런 내 상태만 저장(`user://save.json`). 런 간 유지 데이터용 별도 파일 필요.
...
- 🔲 `save_manager.gd`에 `save_progress()` / `load_progress()` 함수 추가
- 🔲 `autoload/progress_manager.gd` 싱글턴 신설 — 해금 상태 런타임 관리

### 4-2. 챕터 진행 흐름
> 현재 `game_manager.gd`: `current_act`, `MAX_ACTS=3`. 챕터 개념 없음.

- 🔲 `game_manager.gd`에 `current_chapter: int`, `MAX_CHAPTERS: int = 2` 추가
- 🔲 `_end_run_won()` 수정 — 챕터 클리어 처리:
  - `progress_manager.mark_chapter_cleared(current_chapter)` 호출
  - 챕터 클리어 결과 화면 (GameOver 씬 분기 또는 별도 씬)
- 🔲 메인 메뉴에 챕터 선택 UI 추가 (챕터 2는 조건 미충족 시 잠금 표시)
...
- ✅ 기획 문서 작성 완료 (`docs/game_design/enemies_korean_v1.md`, `events_korean_v1.md`, `relics_korean_v1.md`)
  ...
  - 이벤트 풀링: `events_korean.gd` 신화별 파일 분리 (구현 PR에서 `_build_event_pool()` 신화 기반 분기)
  - 챕터 2 운영: 빈 스텁 자동 필터 → 한국만 구현 시 3 Act 한국으로 (중국·일본 추가 시 자동 복귀)
- 🔲 일반 적 6종 구현: 저승사자·처용·도깨비·삼족오·구미호·불가사리
- 🔲 Act 1 엘리트 3종(해치·장승·삼신할머니) + 보스 해모수 (3페이즈, HP 4500)
- 🔲 Act 2 엘리트 3종(도깨비 대장·용왕의 장군·저승 포졸) + 보스 동명성왕 (3페이즈, HP 4800)
- 🔲 Act 3 엘리트 3종(저승 판관·갓신·처용신) + 보스 염라대왕 (3페이즈, HP 4800)
- 🔲 이벤트 10종 구현 (`events_korean.gd`) + `_build_event_pool()` 신화 기반 분기
- 🔲 전용 렐릭 3종 구현: 저승 부적·도깨비 방망이 파편·삼태극 부적

### 중국 신화 (chinese)
- 🔲 일반 적 6종: 야차, 나타 병사, 사대천왕 병사, 산해경 괴수 등
- 🔲 Act 1/2/3 엘리트 3종 + 보스 (예: 서왕모, 천제)
- 🔲 이벤트 10종 (중국 신화 테마)
- 🔲 전용 렐릭 3종

### 일본 신화 (japanese)
- 🔲 일반 적 6종: 오니, 텐구, 유키온나, 갓파, 슈텐도지 졸개, 로닌 망령
- 🔲 Act 1/2/3 엘리트 3종 + 보스 (예: 스사노오, 아마테라스 수호자)
- 🔲 이벤트 10종 (일본 신화 테마)
- 🔲 전용 렐릭 3종

### 챕터 2 공통
- 🔲 챕터 2 신화 랜덤화 (`act_mythologies = ["korean", "chinese", "japanese"].shuffle()`)
...
> 최종 갱신: 2026-04-21. 챕터 시스템 설계 반영.

| 우선순위 | 항목 | 이유 |
|---|---|---|
| 🔴 즉시 | 챕터 시스템 + 메타 진행 저장 (M4-1/4-2) | 모든 콘텐츠 확장의 기반 인프라 |
| 🔴 즉시 | 영웅 해금 시스템 (M4-3/4-4) | 리플레이어빌리티 핵심 — 시작 영웅 선택·영입 풀 확장 |
| 🟠 단기 | 추가 영웅 3명 (M5: 잔 다르크·칭기즈칸·측천무후) | 해금 시스템과 함께 구현 — 시너지 조합 6쌍 추가 |
| 🟠 단기 | 챕터 2 콘텐츠 (M4-2, 한국·중국·일본 신화) | 챕터 시스템 인프라 완성 후 콘텐츠 채우기 |
| 🟡 중기 | 병사 토큰 시스템 (M6-1) | 나폴레옹 군단 아키타입 완성 |
| 🟡 중기 | 카드 타입 시스템 (M6-5, ATTACK/SKILL/POWER) | 렐릭·시너지 트리거 다양화 기반 |
| 🟡 중기 | 시스템 완성 M6 (비밀룸, 부활, 도발, 덱뷰어, 난이도) | 게임 루프 깊이 |
