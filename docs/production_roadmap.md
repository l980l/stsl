...
**설정 옵션 확장 + 카드 v2 사이즈 + i18n 라이브 갱신** (PR #207~#211): MSAA + 윈도우 크기 옵션 + 커스텀 설정 드롭다운 (#210), 카드 v2 native size 1.4x (#208), 언어 변경 시 전투 초기화·상점/영웅 이름 라벨 라이브 갱신 (#207·#209·#211), 전투 폴리시 (피격 방향·sprite Y·turn queue wrap, #211).
**i18n 키워드 namespace 정리** (PR #212·#214): `signature.X.desc`·`battle.signature.X.desc` 동적 합성 (raw 영어 → `%s` placeholder + 헬퍼 args 합성), `keyword.X` 9종 신규 (14언어: attack/block/damage/heal/sacrifice/draw/discard/exhaust/summon), intent 툴팁 damage_type i18n (#214). 가이드 `docs/i18n_keyword_guide.md`.
**몬스터 타입별 공격 VFX + 몬스터 매핑** (PR #213·#215·#216·#217): 신규 공격 VFX 5종 — acid_spray (#213) / needle_sting·bite_attack (#215, 곤충·동물 타입) / claw_attack (#216) / monster_phase_changed (#217). 몬스터 외형 분류 매핑 (동물·곤충·의인화) + 켄타우로스 → 궁수 재해석 (#216), 몬스터 poison 공격 remap (#213), 보스 cinematic = boss grade 한정 (#217). 전갈류 poison → needle_sting (#224).
**적·카드 일러스트 적용** (PR #218·#219): 적 일러스트를 배틀씬 sprite 영역에 cover 모드 적용 + acid_spray VFX 사용 제한 (#218), 카드별 일러스트 `assets/art/cards` 로드 (#219). 다중 페이즈 몬스터 일러스트 가이드 문서 (#205).
...
| ✅ 완료 | 적 패턴 풍부화 — counter_window/SUMMON/MIMIC/silence/heal_block 확장 (M6.22) | PR #185·#186·#187 (Phase A/B/C). **Phase A** — 8 보스 (hydra/hades/kronos/sekhmet/ra_horakhty/fjorgynn/guanyin/jade_emperor) 페이즈 2 에 `CHARGE_UP` + `counter_window_intent.enabled` 추가. 기존 surtr 1명 → **9명** (각 act 1~2). 페이오프 = 각 신화 컨셉 맞춤 (260 LOWEST_HP poison + ALL / 90 ALL curse + DISPEL / 280 ALL holy_fire + vulnerable 3 등). **Phase B** — SUMMON 2 → 4회, MIMIC 1 → 4회: jade_emperor SUMMON 2 celestial_lion (페이즈 1·2, 천계 군단) + twin_immortals MIMIC 35% (쌍둥이 분신) + nine_tailed_spirit MIMIC 30% (여우 분신) + erinyes MIMIC 40% (복수 카르마 반사). 신규 적 생성 X — 기존 normal 재활용. **Phase C** — silence/heal_block 0회 → 각 2회: tengu/dao_disciple `silence 1턴` (음양사 봉인·도교 부적) + apophis_serpent/modgud `heal_block 2턴` (저주의 뱀·Ymir 빙결). 기존 intent 교체 또는 추가 — 신규 적 생성 X. **카운터 카드 활용도 ↑ + 보스 다양성 ↑ + CC 메커니즘 사용 시작**. multi_part 보스 (D)는 별도 작업으로 보류 |
...
| ✅ 완료 | 설정 옵션 확장 + 카드 v2 사이즈 + i18n 라이브 갱신 (M8.7) | PR #207~#211. MSAA + 윈도우 크기 옵션 + 커스텀 설정 드롭다운 (#210) + 카드 v2 native size 1.4x (#208) + 언어 변경 시 전투 초기화·상점/영웅 이름 라벨 라이브 갱신 (#207·#209·#211) + 전투 폴리시 (피격 방향·sprite Y·turn queue wrap, #211) |
| ✅ 완료 | i18n 키워드 namespace 정리 (M8.5-6) | PR #212·#214. `signature.X.desc`/`battle.signature.X.desc` 동적 합성 (raw 영어 → `%s` placeholder + 헬퍼 args) + `keyword.X` 9종 신규 (14언어) + intent 툴팁 damage_type i18n (#214). 가이드 `docs/i18n_keyword_guide.md` |
| ✅ 완료 | 몬스터 타입별 공격 VFX + 몬스터 매핑 (M6.26) | PR #213·#215·#216·#217. 신규 공격 VFX: acid_spray (#213) / needle_sting · bite_attack (#215, 곤충·동물) / claw_attack (#216) / monster_phase_changed (#217). 몬스터 외형 분류 매핑 (동물·곤충·의인화) + 켄타우로스 → 궁수 재해석 (#216) + 몬스터 poison 공격 remap (#213). 보스 cinematic = boss grade 한정 (#217). 전갈류 poison → needle_sting VFX (#224) |
| ✅ 완료 | 적·카드 일러스트 적용 (M7.1) | PR #218·#219. 적 일러스트 sprite 영역 cover 적용 + acid_spray VFX 사용 제한 (#218) + 카드별 일러스트 `assets/art/cards` 로드 (#219). 다중 페이즈 몬스터 일러스트 가이드 (#205) |
| ✅ 완료 | 스토리·로어 + cinematic act-intro (M7.2) | PR #220·#221·#222. 카드 효과 폰트 per-VFX impact 동기화 + 모던 카드 호버 레이아웃 (#220) + 암시적 스토리/로어 (Hall of the Departed / the Cycle, #221) + cinematic act-intro 오버레이 (순차 라인 + 클릭/키 진행, #222) |
| ✅ 완료 | 커스텀 tintable SVG 아이콘 세트 (M7.3) | PR #223. 인텐트·시그니처·승리/패배 아이콘 단색 tintable SVG 전면 교체 + gradient 셰이더 + dark disc 노드 (배경 다양성 대응) + 원형 ring 래핑 + 승리 아이콘 UI 캔버스 배치 + 패배 오버레이 정리 |
| ✅ 완료 | 취약·약화 디버프 VFX + 시그니처 UI 정리 (M6.27) | PR #224. GPU 취약 VFX (혈색/황동 파쇄 + 타겟 중앙 깨진 방패) + GPU 약화 VFX (sage/재 흡수 + 시드는 칼날) + 중앙 `_debuff_script_for_status` resolver 배선 (전 소스) + 떠오르는 Weak/Vulnerable 팝업 색을 VFX 팔레트와 통일 + 저주의 시선 시그니처 타겟 정확화·SFX 소거 + 상태이상 줄에서 신화 시그니처 배지 제거. **1535 통과 / 0 fail** |
