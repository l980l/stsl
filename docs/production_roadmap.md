...
> 기준: 현재 구현 상태(Act 1~2 플레이 가능) 기준 완성까지의 전 작업 목록
> 범례: ✅ 완료 / 🔲 미완료 / 🔶 부분 완료
...
Act 1 + Act 2 플레이 가능 수직 슬라이스 완성.
영웅 3인, 영웅당 카드 40장(아키타입 3개).
Act 1: 적 10종(일반 6·엘리트 4·보스 히드라), 이벤트 10종, 렐릭 19종.
Act 2: 적 10종(일반 6·엘리트 3·보스 오시리스 2페이즈), 이벤트 10종, 전용 렐릭 3종, 난이도 스케일(HP ×1.3, 공격 ×1.2).
