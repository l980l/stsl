...
> 작성일: 2026-04-17 (최종 동기화: 2026-04-21 v2)
> 기준: 챕터 1 완성 + 챕터 2 한국 신화 완료 + 챕터 시스템 인프라 기준
> 범례: ✅ 완료 / 🔲 미완료 / 🔶 부분 완료
...
**챕터 1** (그리스·이집트·북유럽 랜덤 배정) 완성. **챕터 2 한국 신화** 완성 (PR #51).
...
**챕터 2 한국 신화**: 일반 적 6종·Act별 엘리트 9·보스 3(단군/삼신할미/구삼승할망)·이벤트 10종·렐릭 3종(풀 28종). 빈 스텁 자동 필터로 중·일 신화 미구현 시 한국 3 Act 반복.
...
### 챕터 2 공통
- 🔶 빈 스텁 자동 필터 구현 완료 — 현재 한국 신화만 구현됐으므로 3 Act 모두 한국으로 배정. 중·일 추가 시 자동으로 3신화 랜덤화 전환됨
- 🔲 챕터 2 신화 랜덤화 완성 (`act_mythologies = ["korean", "chinese", "japanese"].shuffle()`) — 중·일 신화 구현 완료 후 활성화
...
> 최종 갱신: 2026-04-21 v3. 챕터 시스템·한국 신화 완료 반영 + 영웅 먼저 / 해금 나중 순서 확정.

| 우선순위 | 항목 | 이유 |
|---|---|---|
| 🔴 즉시 | 추가 영웅 3명 (M5: 잔 다르크·칭기즈칸·측천무후) | 해금 시스템의 실효성을 위해 해금 대상 영웅부터 확보. 시너지 조합 6쌍 추가 |
| 🔴 즉시 | 영웅 해금 시스템 (M4-3/4-4) | 영웅 6명 확보 후 구현. ProgressManager API 완료, hero_resource·씬 연결만 남음 |
| 🟠 단기 | 챕터 2 중·일 신화 (M4-2 나머지) | 한국 완료. 중국·일본 각각 일반 적 6종+엘리트 9+보스 3+이벤트 10+렐릭 3 |
