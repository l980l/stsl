...
static func cerberus(scene: PackedScene) -> Resource:
	var e := EnemyRes.new()
	e.enemy_name = "케르베로스"; e.max_hp = 2000; e.character_scene = scene
	e.mythology = "greek"
...
static func charon(scene: PackedScene) -> Resource:
	var e := EnemyRes.new()
	e.enemy_name = "카론"; e.max_hp = 1800; e.character_scene = scene
	e.mythology = "greek"
	var i1 := IntentRes.new()
...
static func erinyes(scene: PackedScene) -> Resource:
	var e := EnemyRes.new()
	e.enemy_name = "에리니에스"; e.max_hp = 1900; e.character_scene = scene
	e.mythology = "greek"
	...
	p0i3.action_type = IntentRes.ActionType.ATTACK; p0i3.value = 140; p0i3.target = IntentRes.TargetType.RANDOM
	var p1i1 := IntentRes.new()
	p1i1.action_type = IntentRes.ActionType.ATTACK; p1i1.value = 180; p1i1.target = IntentRes.TargetType.RANDOM
	var p1i2 := IntentRes.new()
	p1i2.action_type = IntentRes.ActionType.ATTACK; p1i2.value = 180; p1i2.target = IntentRes.TargetType.RANDOM
...
static func hades(scene: PackedScene) -> Resource:
	var e := EnemyRes.new()
	e.enemy_name = "하데스"; e.max_hp = 4800; e.character_scene = scene
	e.mythology = "greek"
	...
	p1i3.action_type = IntentRes.ActionType.ATTACK; p1i3.value = 220; p1i3.target = IntentRes.TargetType.LOWEST_HP
	# 페이즈2 (30% HP 전환)
	...
	p2i2.action_type = IntentRes.ActionType.ATTACK; p2i2.value = 90; p2i2.target = IntentRes.TargetType.ALL
	var p2i3 := IntentRes.new()
	p2i3.action_type = IntentRes.ActionType.ATTACK; p2i3.value = 260; p2i3.target = IntentRes.TargetType.ALL
	e.phase_patterns = [
		[p0i1, p0i2, p0i3],
		[p1i1, p1i2, p1i3],
		[p2i1, p2i2, p2i3]
