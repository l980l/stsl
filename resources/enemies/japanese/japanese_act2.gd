...
static func chaos_tengu(scene: PackedScene) -> Resource:
	var e := EnemyRes.new()
	e.enemy_name = "혼돈의 텐구"; e.max_hp = 1800; e.character_scene = scene
...
static func yasha(scene: PackedScene) -> Resource:
	var e := EnemyRes.new()
	e.enemy_name = "야샤"; e.max_hp = 1900; e.character_scene = scene
	e.mythology = "japanese"
	var i1 := IntentRes.new()
...
static func nureriyon(scene: PackedScene) -> Resource:
	var e := EnemyRes.new()
	e.enemy_name = "누레리온"; e.max_hp = 1700; e.character_scene = scene
	e.mythology = "japanese"
	var i1 := IntentRes.new()
	...
	i5.action_type = IntentRes.ActionType.ATTACK; i5.value = 150; i5.target = IntentRes.TargetType.LOWEST_HP
...
static func shuten_doji(scene: PackedScene) -> Resource:
	var e := EnemyRes.new()
	e.enemy_name = "슈텐도지"; e.max_hp = 4800; e.character_scene = scene
	e.mythology = "japanese"
	...
	p1i1.action_type = IntentRes.ActionType.ATTACK; p1i1.value = 200; p1i1.target = IntentRes.TargetType.LOWEST_HP
	...
	p1i3.action_type = IntentRes.ActionType.ATTACK; p1i3.value = 170; p1i3.target = IntentRes.TargetType.ALL
	...
	p1i5.action_type = IntentRes.ActionType.ATTACK; p1i5.value = 180; p1i5.target = IntentRes.TargetType.RANDOM
	# Phase 2 — 주귀 해방
	...
	p2i4.action_type = IntentRes.ActionType.ATTACK; p2i4.value = 210; p2i4.target = IntentRes.TargetType.ALL
