...
static func jackal_warrior(scene: PackedScene) -> Resource:
	var e := EnemyRes.new()
	e.enemy_name = "자칼 전사"; e.max_hp = 1700; e.character_scene = scene
...
static func scarab_queen(scene: PackedScene) -> Resource:
	var e := EnemyRes.new()
	e.enemy_name = "스카라베 여왕"; e.max_hp = 1600; e.character_scene = scene
	e.mythology = "egyptian"
	var i1 := IntentRes.new()
...
static func obelisk_guardian(scene: PackedScene) -> Resource:
	var e := EnemyRes.new()
	e.enemy_name = "오벨리스크 수호자"; e.max_hp = 2000; e.character_scene = scene
	e.mythology = "egyptian"
...
static func sekhmet(scene: PackedScene) -> Resource:
	var e := EnemyRes.new()
	e.enemy_name = "세크메트"; e.max_hp = 4500; e.character_scene = scene
	e.mythology = "egyptian"
