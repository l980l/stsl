...
static func apophis_serpent(scene: PackedScene) -> Resource:
	var e := EnemyRes.new()
	e.enemy_name = "아포피스의 뱀"; e.max_hp = 2000; e.character_scene = scene
...
static func set_tempest(scene: PackedScene) -> Resource:
	var e := EnemyRes.new()
	e.enemy_name = "세트의 폭풍"; e.max_hp = 2100; e.character_scene = scene
	e.mythology = "egyptian"
...
static func isis_phantom(scene: PackedScene) -> Resource:
	var e := EnemyRes.new()
	e.enemy_name = "이시스의 환영"; e.max_hp = 1950; e.character_scene = scene
	e.mythology = "egyptian"
...
static func ra_horakhty(scene: PackedScene) -> Resource:
	var e := EnemyRes.new()
	e.enemy_name = "라-호라크티"; e.max_hp = 5000; e.character_scene = scene
	e.mythology = "egyptian"
