...
static func apophis_serpent(scene: PackedScene) -> Resource:
	...
	i2.action_type = IntentRes.ActionType.ATTACK; i2.value = 140; i2.target = IntentRes.TargetType.RANDOM; i2.damage_type = "bite_attack"
	...
	i4.action_type = IntentRes.ActionType.ATTACK; i4.value = 140; i4.target = IntentRes.TargetType.RANDOM; i4.damage_type = "bite_attack"
...
static func ra_horakhty(scene: PackedScene) -> Resource:
	...
	# 페이즈 2 — 태양신의 심판 차지업 (counter 가능): 2턴 후 260 ALL fire + vulnerable 3 ALL
	var p2_charge_atk := IntentRes.new()
	p2_charge_atk.action_type = IntentRes.ActionType.ATTACK; p2_charge_atk.value = 260; p2_charge_atk.target = IntentRes.TargetType.ALL; p2_charge_atk.damage_type = "fire"
	...
	e.phase_patterns = [
		...
		[p2i_aff, p2i1, p2_charge, p2i2, p2i3, p2i4]
	]
	e.intent_pattern = e.phase_patterns[0]
	return e
