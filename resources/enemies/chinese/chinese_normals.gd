...
static func heavenly_king_soldier(scene: PackedScene) -> Resource:
	var e := EnemyRes.new()
	e.enemy_name = "enemy.chinese.heavenly_king_soldier"; e.max_hp = 450; e.character_scene = scene
	e.mythology = "chinese"
	var i1 := IntentRes.new()
...
static func shanhaijing_beast(scene: PackedScene) -> Resource:
	var e := EnemyRes.new()
	e.enemy_name = "enemy.chinese.shanhaijing_beast"; e.max_hp = 380; e.character_scene = scene
	e.mythology = "chinese"
	var i1 := IntentRes.new()
...
static func immortal_trainee(scene: PackedScene) -> Resource:
	var e := EnemyRes.new()
	e.enemy_name = "enemy.chinese.immortal_trainee"; e.max_hp = 300; e.character_scene = scene
	e.mythology = "chinese"
	var i1 := IntentRes.new()
...
static func azure_dragon_guard(scene: PackedScene) -> Resource:
	var e := EnemyRes.new()
	e.enemy_name = "enemy.chinese.azure_dragon_guard"; e.max_hp = 520; e.character_scene = scene
	e.mythology = "chinese"
	var i1 := IntentRes.new()
	i1.action_type = IntentRes.ActionType.BUFF; i1.value = 1; i1.status_type = "strength"
	var i2 := IntentRes.new()
