...
func _spawn_peak_burst() -> void:
	...
	var mist := _Helpers.make_emitter({
		"count": _pcount(14), "lifetime": 1.5, "color": COL_GOOP,
...
func _spawn_ambient() -> void:
	...
	_amb_made = true
	var foot: Vector2 = _foot_pos()
	# drip 0.35/frame × 60 × lifetime 1.0 ≈ 21 동시
	_gpu_amb_drip = _Helpers.make_emitter({
		"count": int(21 * _scale()), "lifetime": 1.0, "color": COL_GOOP,
	...
	# mist 0.5/frame × 60 × lifetime 1.7 ≈ 51 동시
	_gpu_amb_mist = _Helpers.make_emitter({
		"count": int(51 * _scale()), "lifetime": 1.7, "color": COL_GOOP,
	...
	# bubble 0.2/frame × 60 × lifetime 0.65 ≈ 8 동시
