...
func _build_ui() -> void:
	...
	if tm:
		for i in range(tm.heroes.size()):
			...
			var current_hp: int = tm.get_hp(hero.hero_id) if tm.has_method("get_hp") else hero.max_hp
			var lbl := Label.new()
	...
	heal_btn.position = Vector2(760, 500)
	heal_btn.size = Vector2(400, 60)
	...
	leave_btn.text = "떠나기"
	leave_btn.position = Vector2(760, 580)
	leave_btn.size = Vector2(400, 60)
